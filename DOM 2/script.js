function createBoard(C = 8, R = 8, collWidth = 100, rowHeight = 100) {
  ...
  for(let new_chess = 0; new_chess < R * 2; new_chess++) {
    document.getElementById("gameBoard").childNodes[new_chess % (R * 2)].style.backgroundImage="url(img/svg/"+color[rnd]+figures[new_chess % (R * 2)]+".svg)";
  }
  for(let new_chess = (R * C) - (R * 2); new_chess < R * C; new_chess++) {
    document.getElementById("gameBoard").childNodes[new_chess].style.backgroundImage="url(img/svg/"+color[rnd]+figures[(new_chess + 8) % (R * 2)]+".svg)";
  }
}
function turn(cell) {
